...
def but_1():  # когда нажимается кнопка1, закрывается одно окно и создается второе с игрой
    ...
    lab1 = Label(tk)
    lab1.place(relx=0.12, rely=0.6, anchor=CENTER) #Первая картинка куба
    lab2 = Label(tk)
    lab2.place(relx=0.32, rely=0.6, anchor=CENTER) #Вторая картинка куба
    lab3 = Label(tk)
    lab3.place(relx=0.68, rely=0.6, anchor=CENTER)  # Первый куб бота
    lab4 = Label(tk)
    lab4.place(relx=0.88, rely=0.6, anchor=CENTER)  # Второй куб бота



    def bros():
        x = random.choice(["kube1.png", "kube2.png", "kube3.png", "kube4.png", "kube5.png", "kube6.png"])
    ...
    def new_game(): #Часть ответственная за смену картинок
        global b1, b2, b3, b4
        for i in range(10):
            b1 = PhotoImage(file=(bros())).subsample(2, 2)
            b2 = PhotoImage(file=(bros())).subsample(2, 2)
            b3 = PhotoImage(file=(bros())).subsample(2, 2)
            b4 = PhotoImage(file=(bros())).subsample(2, 2)
            lab1["image"] = b1
            lab2["image"] = b2
            lab3["image"] = b3
            lab4["image"] = b4
...
def but_2():  # когда нажимается кнопка2, закрывается одно окно и создается второе с игрой
    ...
    def new_game2():  # Часть ответственная за смену картинок
        global b3, b4, block1, block2, sum1
        if block2 == 0:
            for i in range(10):
                b3 = 1
                b4 = 1
    ...
    Button(tk, text="Статистика", comman=stat).place(x=100, y=60)  # Кнопка статистики
